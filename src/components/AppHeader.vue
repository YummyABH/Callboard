...
<template>
	...
	<header id="#top" class="fixed left-0 top-0 z-50 bg-black-400  w-full min-h-16 shadow-2xl">
		<ContentContainer>
			<nav class="flex items-center h-24 z-20 gap-x-5 max-xl:justify-between max-xl:relative">
				<RouterLink to="/" class=" hover:bg-inherit z-30 flex items-center">
					<svg xmlns="http://www.w3.org/2000/svg" height="50px" viewBox="178.4 50.1 306.7 408.4">
						<g fill="#8c7b27">
							<path
				...
				<div class="w-full max-lg:ml-5  max-sm:ml-2 ml-10 z-10 flex justify-between items-center">
					<Search class="z-20 h-7 w-7" />
					<MenuNavigation v-model="isOpen" />
					<ButtonBurger v-model="isOpen" />
				</div>
			</nav>
		</ContentContainer>
	</header>
