...
<template>
	<form class=" text-white flex justify-between overflow-hidden bg-black-299 rounded-xl">
		<input v-model="textSearch" type="text" autocomplete="off" name="search"
			class="appearance-none border-none outline-none bg-inherit h-6 px-5 py-6 w-full" placeholder="Найти" id="1">
		<button @click.prevent="APIFilterName()"
			class="flex text-xl max-md:px-6  px-10 bg-gray-700  items-center">Найти</button>
	</form>
</template>