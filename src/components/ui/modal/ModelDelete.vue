...
import { useAdDelateApi } from '@/API/delateAd'

const { deleteAd } = useAdDelateApi()
...
<template>
  ...
  <div
    class="overflow-hidden bg-black-301 pb-6 pt-12 px-20 flex items-center justify-center fixed rounded z-51 left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2"
  >
    <div
      @click="isOpen = !isOpen"
    ...
    <form class="flex flex-col items-center gap-4">
      <label for="code" class="text-xl max-md:text-base">Удалить объявление ?</label>
      <div class="flex gap-4">
        <button
          @click.prevent="deleteListing(activeId)"
          type="submit"
          class="px-2 py-2 cursor-pointer hover:bg-red-600 active:bg-red-900 text-white duration-200 rounded bg-red-700"
        >
          Подтвердить
        </button>
        <button
          @click="isOpen = !isOpen"
          type="submit"
          class="px-2 py-2 text-white cursor-pointer hover:bg-green-400 active:bg-green-300 duration-200 rounded bg-green-500"
        >
          Отмена
        </button>
      </div>
